...
- [x] **BENCH-01**: Benchmark scripts compare secured vs standard JHipster Organization endpoints at 1, 10, and 50 virtual users and produce persisted Markdown summaries including p95 overhead against the < 10% target.
- [x] **OPENAPI-01**: In-scope secured and security endpoints expose accurate OpenAPI annotations including fetch-plan notes, response schema typing for `ResponseEntity<String>` JSON bodies, and `x-secured-entity` markers for secured entity operations.
...
*Last updated: 2026-04-01 after benchmark and documentation closure (PERF-04, BENCH-01, OPENAPI-01 complete)*
