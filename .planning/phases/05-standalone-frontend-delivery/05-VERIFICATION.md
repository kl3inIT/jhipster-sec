...
verified: 2026-03-21T20:13:02.1874470Z
status: human_needed
...
| `frontend/src/app/pages/admin/security/permission-matrix/permission-matrix.component.ts` | Two-panel matrix editor using the locked UI contract | VERIFIED | Uses role-route params, filtered permission queries, guarded pending state, and `GRANT` payloads. |
...
| `frontend/src/app/pages/admin/security/permission-matrix/permission-matrix.component.ts` | `frontend/src/app/pages/admin/security/shared/service/sec-permission.service.ts` | filtered query plus create and delete toggles | WIRED | The matrix loads role-scoped permissions, guards pending writes, and issues create/delete requests immediately per toggle. |
...
**Expected:** CRUD changes persist, the matrix still speaks `GRANT` at the UI boundary, and protected-entity behavior changes accordingly for the affected user.
...
The prior code gaps are closed. Unknown routes now land on the 404 page, the permission matrix contract is normalized end to end without changing the locked frontend payload shape, and the protected-entity screens now consume shared capability data to gate actions, edit buttons, route entry, and sensitive fields.
...
_Verified: 2026-03-21T20:13:02.1874470Z_
